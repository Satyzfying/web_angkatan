  ...
  return createPortal(
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto px-4 py-[5dvh]">
      ...
      <div style={{ backgroundImage: `url(${bgCard.src || bgCard})` }}
           className="border-[#3B2D21] relative z-10 max-h-[90dvh] w-full max-w-[720px] animate-[member-popup-show_200ms_ease-out] overflow-y-auto rounded-2xl border-4 bg-cover bg-center bg-no-repeat p-6 text-white shadow-xl sm:p-8">
        <button
          type="button"
          aria-label="Close member detail"
          onClick={onClose}
