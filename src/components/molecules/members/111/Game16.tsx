...
export default function Game16({ onWin }: Game16Props) {
  ...
  const getBgColor = (val: number) => {
    switch (val) {
      ...
      case 8:
        return 'bg-white/30 text-white/95 drop-shadow-md'
      case 16:
        return 'bg-emerald-500/80 text-white font-black drop-shadow-lg'
      default:
        return 'bg-black/40'
    }
  }
  ...
  return (
    <div className="flex w-full flex-col items-center rounded-2xl border border-white/20 bg-black/20 p-5 backdrop-blur-md">
      <p className="mb-4 text-center text-sm font-semibold text-white/80 drop-shadow-[0_1px_2px_rgba(0,0,0,1)]">
        ayo <span className="font-bold text-emerald-400">16</span> point masa gabisa <br />
        <span className="text-xs text-white/50">(ea ea ea ea)</span>
      </p>

      {/* Area yang bisa diusap/swipe di HP */}
      <div
        className="grid touch-none grid-cols-4 gap-2 rounded-xl border border-white/10 bg-black/40 p-2"
      ...
      >
        {board.map((row, r) =>
          row.map((cell, c) => (
            <div
              key={`${r}-${c}`}
              className={`flex h-12 w-12 items-center justify-center rounded-lg text-lg font-bold transition-all duration-150 sm:h-14 sm:w-14 ${getBgColor(cell)}`}
            >
              {cell !== 0 ? cell : ''}
            </div>
          ))
      ...
      <button
        ...
        className="mt-5 rounded-full border border-white/10 bg-white/10 px-4 py-2 text-xs text-white/70 transition-colors hover:bg-white/20"
      >
        gabisa=bot
      </button>
    </div>
