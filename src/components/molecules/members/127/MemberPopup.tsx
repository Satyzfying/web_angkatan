...
const STATS = [
  { label: 'Introverted',  value: 58, color: '#22d3ee', shadow: 'rgba(34,211,238,0.8)',  roast: 'needs 3 days to recover from one hangout 💀' },
  { label: 'Extroverted',  value: 52, color: '#60a5fa', shadow: 'rgba(96,165,250,0.8)',  roast: 'extrovert era... for exactly 20 minutes then bye 👋' },
  { label: 'Asbun',        value: 89, color: '#c084fc', shadow: 'rgba(192,132,252,0.8)', roast: 'speaks first, thinks never. legend behavior 🎤' },
  { label: 'Dream',        value: 92, color: '#f472b6', shadow: 'rgba(244,114,182,0.8)', roast: 'dreams big, naps bigger 🌙' },
  { label: 'Budget Left',  value: 3,  color: '#f87171', shadow: undefined,               roast: 'literally surviving on prayers and free campus wifi 🙏' },
  { label: 'Coding Skill', value: 76, color: '#4ade80', shadow: 'rgba(74,222,128,0.8)',  roast: 'could ship it. will it compile? mystery 🎲' },
]
...
const MemberPopup = ({ isOpen, onClose }: MemberPopupProps) => {
  ...
  useEffect(() => {
    ...
    const spawnMeteor = () => meteors.push({
    ...
    const draw = () => {
      ...
      for (let i = meteors.length - 1; i >= 0; i--) {
        ...
        m.x += dx * m.speed; m.y += dy * m.speed; m.life++
  ...
  // Typing effect
  useEffect(() => {
    if (!isOpen) { setTypedQuote(''); return }
    let i = 0
    setTypedQuote('')
    const type = () => {
      if (i <= QUOTE.length) {
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto px-4 pt-28 pb-8 sm:pt-32">
      ...
      <div className="relative z-10 max-h-[calc(100vh-9rem)] w-full max-w-[740px] animate-[member-popup-show_200ms_ease-out] overflow-y-auto rounded-2xl border-2 border-white/10 bg-[linear-gradient(135deg,#020617_0%,#0f172a_45%,#1e1b4b_100%)] p-6 text-white shadow-[0_0_60px_rgba(59,130,246,0.3)] sm:max-h-[calc(100vh-10rem)] sm:p-8">
        ...
        <canvas ref={canvasRef} aria-hidden="true"
          className="pointer-events-none absolute inset-0 h-full w-full rounded-2xl" style={{ zIndex: 0 }} />

        {/* Ambient glows */}
        <div aria-hidden="true" className="pointer-events-none absolute -left-20 -top-20 h-80 w-80 rounded-full bg-cyan-500/15 blur-3xl" />
        <div aria-hidden="true" className="pointer-events-none absolute right-0 top-16 h-64 w-64 rounded-full bg-purple-500/15 blur-3xl" />
        <div aria-hidden="true" className="pointer-events-none absolute bottom-0 left-1/3 h-72 w-72 rounded-full bg-blue-500/15 blur-3xl" />

        {/* Twinkle dots */}
        <div aria-hidden="true" className="absolute top-8 left-8 h-1.5 w-1.5 rounded-full bg-cyan-300 animate-ping" />
        <div aria-hidden="true" className="absolute top-36 right-20 h-1 w-1 rounded-full bg-white animate-pulse" />
        <div aria-hidden="true" className="absolute bottom-28 left-1/2 h-1.5 w-1.5 rounded-full bg-blue-300 animate-ping" />
        ...
        <div className="relative z-10">
          ...
          {/* Close button */}
          <button type="button" aria-label="Close member detail" onClick={onClose}
            className="absolute top-0 right-0 flex h-9 w-9 items-center justify-center rounded-full border border-white/20 text-lg leading-none hover:bg-white/10 transition-colors">
            ×
          </button>

          {/* Profile photo */}
          <div className="mb-5 overflow-hidden rounded-2xl border border-white/10">
            <Image src={ProfileImage} alt="Foto profil Razana Aulia"
          ...
          <div className="pr-10">
            ...
            <p className="mt-3 text-sm italic text-blue-300 min-h-[1.5rem]">
              {typedQuote}
              <span className="inline-block w-[2px] h-[0.9em] bg-blue-300 ml-[1px] align-middle animate-pulse" />
            </p>
          </div>
          ...
          <div className="mt-4 rounded-xl border border-emerald-400/25 bg-slate-900/50 p-4 backdrop-blur-md">
            ...
            <p className="mt-1.5 text-sm italic text-emerald-300">Surviving on vibes and delusion.</p>
          </div>
          ...
          <div className="mt-5 grid gap-4 text-sm font-semibold sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-xl border border-white/10 p-4">
                <p className="text-xs uppercase tracking-wide text-white/50">Hobi</p>
              ...
              <div className="rounded-xl border border-cyan-400/25 bg-slate-900/50 p-4 backdrop-blur-md">
                <p className="text-xs font-bold uppercase tracking-widest text-cyan-300">Profile</p>
                <div className="mt-3 space-y-1.5 text-sm">
                  <p><span className="text-cyan-400">CLASS</span> : Professional Overthinker</p>
                  <p><span className="text-cyan-400">RARITY</span> : Epic</p>
                  <p><span className="text-cyan-400">STATUS</span> : Still Surviving</p>
                </div>
              </div>
              ...
              <div className="relative overflow-hidden rounded-xl border border-blue-400/25 bg-slate-900/50 p-4 backdrop-blur-md">
                <svg aria-hidden="true"
                  className="pointer-events-none absolute right-0 top-0 h-full w-36 opacity-20"
                  viewBox="0 0 90 160" xmlns="http://www.w3.org/2000/svg">
                ...
                <p className="text-xs font-bold uppercase tracking-widest text-blue-300">System Status</p>
            ...
            <div className="flex flex-col gap-4">
              ...
              {/* Fun Fact */}
              <div className="rounded-xl border border-white/10 p-4">
                <p className="text-xs uppercase tracking-wide text-white/50">Fun Fact</p>
                <p className="mt-2">My stomach rejects Soto every single time</p>
              </div>

              {/* Character Sheet */}
              <div className="rounded-xl border border-cyan-400/25 bg-slate-900/50 p-4 backdrop-blur-md">
                <p className="text-xs font-bold uppercase tracking-widest text-cyan-300">Character Sheet</p>
                <p className="mt-1 text-[10px] text-white/30 uppercase tracking-wide">klik bar buat roast 👆</p>
                <div className="mt-3 space-y-4">
                  {STATS.map(({ label, value, color, shadow, roast }, idx) => (
                    <div key={label}>
                      ...
                      >
                        <div className="h-2.5 overflow-hidden rounded-full bg-slate-700/80 group-hover:brightness-110 transition-all">
                          <div
                      ...
                      {activeRoastIdx === idx && (
                        <p className={`${styles.roastIn} mt-1.5 text-[11px] italic text-white/70 pl-1`}>
                          → {roast}
                        </p>
                      )}
                    </div>
          ...
          <div className="mt-4 rounded-xl border border-purple-400/25 bg-slate-900/50 p-4 backdrop-blur-md">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs font-bold uppercase tracking-widest text-purple-300">Ask Tejan</p>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-purple-400/50 transition-colors"
              />
              <button
                type="button"
                onClick={handleAsk}
                className={`rounded-lg border border-purple-400/40 bg-purple-500/20 px-4 py-2 text-sm font-bold text-purple-300 hover:bg-purple-500/30 transition-colors ${askShaking ? styles.shake : ''}`}
              >
                Tanya
              </button>
            </div>
            {askAnswer && (
              <div className={`${styles.answerIn} mt-3 rounded-lg border border-purple-400/20 bg-purple-900/20 px-3 py-2.5 text-sm italic text-purple-200`}>
                "{askAnswer}"
              </div>
            )}
          </div>
          ...
          <div className="mt-4 rounded-xl border border-blue-400/25 bg-slate-900/50 p-4 backdrop-blur-md">
            ...
            <p className="mt-1 text-[10px] text-white/30 uppercase tracking-wide">klik lagu buat expand player 🎵</p>
                ...
                return (
                  <div key={song.title}>
                    <button
                      ...
                      className="w-full flex items-center gap-3 rounded-lg border border-white/5 bg-white/5 px-3 py-2.5 hover:bg-white/10 transition-all text-left"
                    >
                      <span className="text-xs text-white/40 w-4 flex-shrink-0 text-center">
                        {isActive ? '▶' : idx + 1}
                      </span>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold truncate" style={{ color: isActive ? song.color : undefined }}>
                          {song.title}
                        </p>
                    ...
                    {isActive && (
                      <div className={`${styles.answerIn} mt-1 rounded-lg overflow-hidden border border-white/5`}>
